const productSections = [
  ...
  const ProductGridSections = () => {
    return (
      <section className="container mx-auto py-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-10">
        {productSections.map((section, idx) => (
          <div key={idx}>
            <h3 className="text-lg font-semibold mb-4">{section.title}</h3>
            <div className="space-y-4">
              {section.products.map((item, i) => (
                <div key={i} className="border p-3 rounded-lg shadow-sm hover:shadow-md transition">
